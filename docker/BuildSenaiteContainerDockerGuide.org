...
modify your develop-compose.yml so that Senaite starts in foreground mode. You need to include the 'fg' command as shown;
  ...
  #+end_src
To determine if you have successfully launched Senaite in foreground mode, navigate to http://127.0.0.1:8080/senaite/@@overview-controlpanel. The botton of the page will state that 'you are running "debug mode"'. [[Link]
